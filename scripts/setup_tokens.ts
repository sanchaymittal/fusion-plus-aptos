...
async function setupTokens() {
    ...
    try {
        ...
        if (aptCoinStore && aptCoinStore.data) {
            ...
            console.log(`💰 APT Balance: ${balance.toString()} octas (${(Number(balance) / 100_000_000).toFixed(4)} APT)`)
        }
        ...
        if (tokenCoinStore && tokenCoinStore.data) {
            ...
            const balance = BigInt(data.coin.value || '0')
            console.log(`🪙 Custom Token Balance: ${balance.toString()} units (${(Number(balance) / 100_000_000).toFixed(4)} tokens)`)
        }
